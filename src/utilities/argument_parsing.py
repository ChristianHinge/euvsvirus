...
def delimiter(parser):
    parser.add_argument("-d", "--sep", default=None, dest="delimiter",
                        help="Delimiter character to separate values in file. Default=whitespace.")
...
def parse_inoutfile(args):
    ...
    if len(args.inoutfile) == 1:
        # is the one provided in/out an in or an out?
        if io.is_reading_from_pipe():
            args.inoutfile = [sys.stdin, args.inoutfile[0]]
        else:
